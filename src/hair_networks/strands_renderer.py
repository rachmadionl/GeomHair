...
# RefinerUNet (from the npbgpp submodule) is only used by Renderer, which is not
# instantiated in the current pipeline. Import lazily so the npbgpp submodule is
# only required when the npbgpp-based Renderer is actually used.
try:
