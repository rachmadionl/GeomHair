...
# LAVIS is vendored under submodules/; k_diffusion is provided by the pip package.
sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..', 'submodules', 'LAVIS'))
from k_diffusion.sampling import get_sigmas_karras
from lavis.models import load_model_and_preprocess

from src.utils.config import make_model, make_denoiser_wrapper, make_sample_density
...
from src.diffusion_prior.sampler import sample_euler_ancestral
...
class OptimizableTexturedStrands(nn.Module):
    def __init__(self, 
        ...
        scalp_uvs = torch.load('./data/symmetry_scalp_uvcoords.pth').cuda()[None] # generated in Blender uv map for the scalp
        ...
        if diffusion_cfg['use_diffusion'] and diffusion_cfg['model']['type'] == 'openai':
            mgrid = torch.stack(
                torch.meshgrid([torch.linspace(-1, 1, texture_size)] * 2, indexing='xy')
            )[None].cuda()
        else:
            mgrid = torch.stack(
                torch.meshgrid([torch.linspace(-1, 1, texture_size)] * 2)
            )[None].cuda()
        ...
        print('use diffusion prior: ', self.use_diffusion)
        ...
        if self.use_diffusion:
            ...
            inner_model = make_model(diffusion_cfg)
            ...
            ckpt = torch.load(diffusion_cfg['model']['dif_path'], map_location='cpu')
            self.accelerator.unwrap_model(self.model_ema.inner_model).load_state_dict(ckpt['model_ema'])
            param_to_buffer(self.model_ema)

            self.model_type = diffusion_cfg['model']['type']
            self.diffusion_input = diffusion_cfg['model']['input_size'][0]
            self.sample_density = make_sample_density(diffusion_cfg['model'])
            self.start_denoise = diffusion_cfg['start_denoise']
            self.diffuse_bs = diffusion_cfg['diffuse_bs']
            self.sigma_min = diffusion_cfg['model']['sigma_min']
            self.sigma_max = diffusion_cfg['model']['sigma_max']
            self.diffuse_iter = diffusion_cfg['diffuse_iter']
            self.n_diffuse_steps = diffusion_cfg['n_diffuse_steps']
            self.model_type = diffusion_cfg['model']['type']
            if self.model_type == 'openai':
                self.cfg_scale = diffusion_cfg['cfg_scale']
            else:
                self.cfg_scale = 0.0
            print('cfg_scale:', self.cfg_scale)
            print('diffusion steps:', self.diffuse_iter)
            print('n_diffuse_steps:', self.n_diffuse_steps)
            print('Model type: ', self.model_type)
            if self.diffuse_iter > 0:
                self.all_noise_levels = get_sigmas_karras(
                    self.diffuse_iter, float(self.sigma_min), float(self.sigma_max), rho=7., device=mesh.device
                )
            else:
                self.all_noise_levels = None
            ...
            if self.diffuse_mask: 
                self.diffuse_mask = torch.tensor(cv2.imread(self.diffuse_mask))[:, :, :1].squeeze(-1).cuda()
                if self.diffuse_mask.max() == 255:
                    self.diffuse_mask = self.diffuse_mask / 255.0
                print(f'min max mask: {self.diffuse_mask.min()}, {self.diffuse_mask.max()}')

            if self.model_type == 'openai':
                self.precompute_cond = diffusion_cfg.get('precompute_condition', None)
                if self.precompute_cond:
                    front_ckpt = os.path.join(self.precompute_cond, 'frontal.pt')
                    back_ckpt = os.path.join(self.precompute_cond, 'back.pt')

                    front_emb = torch.load(front_ckpt, map_location=head_mesh.device)
                    back_emb = torch.load(back_ckpt, map_location=head_mesh.device)

                    self.cross_cond = torch.concat(
                        (front_emb, back_emb), 0
                    ).to(head_mesh.device).mean(0, keepdim=True)[None]
                    if len(self.cross_cond.shape) == 4:
                        self.cross_cond = self.cross_cond.squeeze(0)
                else:
                    # BLIP text embeddings
                    self.model_feature_extractor, _, self.txt_processors = load_model_and_preprocess(
                        name="blip2_feature_extractor",
                        model_type="pretrain",
                        is_eval=True,
                        device=head_mesh.device,
                    )

                    self.hairstyle_description = "long curly hairstyle"
                    print(f"Prompt used: {self.hairstyle_description}")
                    self.cross_cond = obtain_description_embedding(
                            hairstyle_description=self.hairstyle_description,
                            average_descriptions=True,
                            model_feature_extractor=self.model_feature_extractor,
                            txt_processors=self.txt_processors
                        )
    
    def get_appropriate_sigmas(self, it, diffusion_texture, sigma):
        base_params = {
            'rho': 7.,
            'device': diffusion_texture.device,
            'sigma_min': float(self.sigma_min),
            'sigma_max': sigma[0].item()
        }
        
        steps = 1 if it > self.diffuse_iter and self.n_diffuse_steps >= 2 else self.n_diffuse_steps
        return get_sigmas_karras(steps, **base_params)
    ...
    def forward(self, it=None): 
        ...
        if self.use_diffusion and it is not None and it >= self.start_denoise:
            ...
            if (self.all_noise_levels is not None) and (it <= self.diffuse_iter):
                sigma = self.all_noise_levels[it - self.start_denoise][None].repeat(diffusion_texture.shape[0])
            else:
                sigma = self.sample_density([diffusion_texture.shape[0]], device=diffusion_texture.device)

            noised_texture = diffusion_texture + noise * sigma
            sigmas = self.get_appropriate_sigmas(it, diffusion_texture, sigma)

            extra_args = {}
            if self.model_type == 'openai':
                extra_args['cross_cond'] = self.cross_cond.repeat(
                    diffusion_texture.shape[0], *[1] * len(self.cross_cond.shape[1:])
                )

            denoised_texture = sample_euler_ancestral(
                self.model_ema, noised_texture, sigmas, extra_args=extra_args, cfg_scale=self.cfg_scale, disable=True
            ).detach()
            mask = None
            ...
            if mask is not None:
                L_diff = (((denoised_texture - diffusion_texture) ** 2) * mask / 2)
            else:
                L_diff = (((denoised_texture - diffusion_texture) ** 2) / 2)

            # L_diff, pred_image, noised_image = self.model_ema.loss_wo_logvar(
            #     diffusion_texture,
            #     noise,
            #     sigmas,
            #     sigmas[0],
            ...
            #     mask=mask,
            #     unet_cond=None,
            #     num_steps=self.diffuse_iter
            # )

            diffusion_dict['L_diff'] = L_diff
        ...
        return p, z_geom, z_app, idx, diffusion_dict
