...
def sample_gt_orientations(orient_2d_cam, gt_orient_map, orient_mask, orient_conf):
    height, width = gt_orient_map.shape[2:]  # Assuming gt_orient_map is of shape [N, C, H, W].
    ...
    sampled_orientations = sampled_orientations.view(-1, 1)
    sampled_mask = sampled_mask.view(-1, 1)
    sampled_conf = sampled_conf.view(-1, 1)
    return sampled_orientations, sampled_mask, sampled_conf
...
def project_orient_to_camera(orient_3d, org_3d, cam_intr, cam_extr):
    ...
    sampled_orient_angle = torch.acos(orient_cos.clamp(-1 + 1e-5, 1 - 1e-5))  # output in rad
    ...
    orient_angle = orient_angle[:, None]  # M x 1
    return orient_angle, org_2d_cam


def align_projected_and_gt_orientations(
        orient_3d, org_3d, cam_intr, cam_extr, gt_orient_map, orient_mask, orient_conf
    ):
    orient_angle, projected_2d_points = project_orient_to_camera(
        orient_3d, org_3d, cam_intr, cam_extr
    )
    
    gt_orient_angle, orient_mask_sampled, orient_conf_sampled = sample_gt_orientations(
        projected_2d_points, gt_orient_map, orient_mask, orient_conf
    )
    return orient_angle, gt_orient_angle, orient_mask_sampled, orient_conf_sampled, projected_2d_points
